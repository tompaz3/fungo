...
import (
	...
	errmatch "github.com/tompaz3/fungo/match/error/patternmatch"
)
...
var _ = g.Describe("Switch", func() {
			...
			g.Context("and matched function returns value", func() {
				g.It("should retrieve value", func() {
					...
					res, err := errmatch.Switch(
						receivedErr,
						errmatch.CaseType(func(_ PaymentGatewayRejectedError) (int, error) {
							executedMethods = append(executedMethods, "PaymentGatewayRejectedError")
							return 1, nil
						}),
						errmatch.CaseType(func(_ InsufficientFundsError) (int, error) {
							executedMethods = append(executedMethods, "InsufficientFundsError")
							return 2, nil
						}),
						errmatch.CaseType(func(_ ProductNotFoundError) (int, error) {
							executedMethods = append(executedMethods, "ProductNotFoundError")
							return 3, nil
						}),
						errmatch.CaseType(func(_ UnsupportedCurrencyError) (int, error) {
	...
	g.Describe("Case", func() {
		g.When("typed err predicate used", func() {
			g.It("should match", func() {
				executedMethods := make([]string, 0)
				res, err := errmatch.Switch(
					receivedErr,
					errmatch.Case(
						errmatch.Type[PaymentGatewayRejectedError]().
							And(func(err PaymentGatewayRejectedError) bool {
								return err.ReasonCode == "100"
							}),
						func(_ PaymentGatewayRejectedError) (int, error) {
							executedMethods = append(executedMethods, "PaymentGatewayRejectedError")
							return 1, nil
						},
					),
					errmatch.Case(
						errmatch.Matches(func(err error) bool {
							return err.Error() == "insufficient amount for payment 100.00"
						}),
